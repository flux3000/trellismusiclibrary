...
def analyze():
    ...
    _QUALITY_JOBS[job_id] = {
        ...
        # The folders THIS scan resolved, normalised for comparison against
        # staging rows.  `analyze_status` filters its result set to these —
        # see the note there.  A list, not a set, only because job state is
        # occasionally dumped to the debug drawer as JSON.
        "paths": [qs.norm_path(f) for f in folders],
    }
    threading.Thread(
...
def analyze_status(job_id):
    ...
    # Narrow to the folders THIS scan actually resolved on disk.
    #
    # `list_staging` answers "every unpromoted row recorded under this source
    # dir", which is a superset of "what is in this directory now" and drifts
    # further from it with every run.  A row outlives its folder on purpose
    # (a Move ingest relocates the source out from under it), so a show
    # ingested from a DIFFERENT path leaves its second, never-promoted row
    # behind — and that row then reappeared in the queue on every later scan
    # of the same parent, wearing "Folder moved" and offering nothing to do.
    # Ryan, 2026-09-02: a Pat Metheny show from several jobs ago that would
    # not go away.  It was row 127, `.../Trellis/Download/Pat Metheny Group
    # 1979-06-14 Philadelphia, PA`; the same show had been ingested as
    # recording 571 under its old Flux Workshop path, so the row under
    # Download was never marked promoted and nothing else would ever clear it.
    #
    # The job already knows the answer — `resolve_shows()` produced it before
    # the first byte was decoded — so the review list is scoped to that,
    # rather than to whatever the table happens to remember.  Nothing is
    # deleted: a row for a folder that is genuinely still there is unaffected,
    # and `scripts/prune_orphan_staging.py` cleans up the historical leftovers
    # separately.
    paths = job.get("paths")
    if paths is not None:
        # Normalised on BOTH sides, even though `analyze()` already normalises
        # what it stores. macOS hands back decomposed filenames and SQLite
        # stores composed ones, so a byte comparison here would drop every
        # accented folder from the queue — and it would arrive as an empty
        # list, not an error, which is the one failure shape this whole route
        # keeps having to be taught not to produce.  A filter that depends on
        # who normalised first is a filter waiting to be broken by its next
        # caller.
        wanted = {qs.norm_path(p) for p in paths}
        rows = [r for r in rows if qs.norm_path(r.folder_path) in wanted]
